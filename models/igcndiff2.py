...
class GCNdiff(nn.Module):
    def __init__(self, adj, config):
        ...
        logging.info(f"IGCNdiff DEQ components created: {len(self.deq_manager.components)} registered")
    ...
    def forward(self, x, mask, t, cemd):
        ...
        if self.use_middle_layer_deq and hasattr(self, 'deq_block'):
            try:
                ...
                #logging.info(f"DEQ middle layer performed {iterations} iterations")
